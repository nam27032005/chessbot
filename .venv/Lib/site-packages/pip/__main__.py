...
class ChessGame:
    def __init__(self, root):
        self.root = root
        self.root.title("Chess Bot with Stockfish")
        self.board = chess.Board()
        self.move_stack = []  # Lưu trữ lịch sử nước đi để hỗ trợ Undo
        ...
        self.stockfish_path = "C:/Users/ADMIN/PycharmProjects/chessbot/stockfish/stockfish-windows-x86-64-avx2.exe"
        if not os.path.exists(self.stockfish_path):
            messagebox.showerror("Error", f"Stockfish file not found at: {self.stockfish_path}\nPlease download Stockfish and place it in the 'stockfish' folder.")
            self.root.destroy()
            return

        try:
            self.engine = chess.engine.SimpleEngine.popen_uci(self.stockfish_path)
            self.engine.configure({"Threads": 2, "Hash": 2048, "Skill Level": 20})
        except Exception as e:
            messagebox.showerror("Error", f"Failed to initialize Stockfish: {e}")
            self.root.destroy()
            return
        ...
        self.canvas = tk.Canvas(self.game_frame, width=480, height=480)
        self.canvas.pack()
        self.canvas.bind("<Button-1>", self.player_move)

        self.status_label = tk.Label(self.game_frame, text="", font=("Arial", 12))
        self.status_label.pack()

        self.eval_label = tk.Label(self.game_frame, text="Evaluation: N/A", font=("Arial", 12))
        self.eval_label.pack()

        self.turn_label = tk.Label(self.game_frame, text="Turn: White", font=("Arial", 12))
        self.turn_label.pack()

        self.timer_label = tk.Label(self.game_frame, text="Player Time: 10s", font=("Arial", 12))
        ...
        self.difficulty_label = tk.Label(self.game_frame, text="Bot Difficulty (Skill Level 0-20):", font=("Arial", 12))
        self.difficulty_label.pack()
        self.difficulty_scale = tk.Scale(self.game_frame, from_=0, to=20, orient=tk.HORIZONTAL, command=self.update_difficulty)
        self.difficulty_scale.set(20)  # Mặc định là mức mạnh nhất
    ...
    def update_difficulty(self, value):
        skill_level = int(value)
        try:
            self.engine.configure({"Skill Level": skill_level})
            self.status_label.config(text=f"Bot difficulty set to Skill Level {skill_level}")
        except Exception as e:
            self.status_label.config(text=f"Error setting difficulty: {e}")
        ...
        with self.lock:
            if move and self.bot_thinking:
                if move in self.board.legal_moves:
                    self.move_stack.append(self.board.fen())  # Lưu trạng thái bàn cờ trước khi đi
    ...
    def undo_move(self):
        if not self.bot_thinking and len(self.move_stack) > 0:
            ...
            if self.bot_color == self.player_color:
                # Nếu người chơi vừa đi, hoàn tác thêm nước đi của bot
    ...
    def highlight_legal_moves(self, square):
        self.update_board()  # Vẽ lại bàn cờ
